<?php
/**
 * Generate Contract Preview PDFs
 * This generates preview PDFs with customer data and "SIGNATURE PENDING" watermarks
 */
...
if (!in_array($contract_type, ['croa', 'agreement', 'poa'])) {
...
try {
    ...
    // Determine which package to use based on state
    $package_id = null;
    $package = null;
    ...
    } elseif ($contract_type === 'poa') {
        // Power of Attorney only
        $stmt = $pdo->prepare("SELECT * FROM state_contract_documents WHERE package_id = ? AND contract_type = 'power_of_attorney'");
        $stmt->execute([$package['id']]);
        $document = $stmt->fetch();

        if (!$document) {
            throw new Exception("Power of Attorney document not found");
        }
        $documents_to_process[] = $document;

    } else { // agreement - include client_agreement and notice_of_cancellation (with duplicate)
    ...
    foreach ($documents_to_process as $document) {
        ...
        if ($document['contract_type'] === 'croa_disclosure') {
            $form_data['client_name'] = $enrollment['first_name'] . ' ' . $enrollment['last_name'];
            $form_data['enrollment_date'] = date('m/d/Y');

        } elseif ($document['contract_type'] === 'client_agreement') {
            $form_data['client_name'] = $enrollment['first_name'] . ' ' . $enrollment['last_name'];
            $form_data['client_address'] = trim(
            ...
            $form_data['client_phone'] = format_phone($enrollment['phone']);
            $form_data['client_email'] = $enrollment['email'];
            $form_data['enrollment_date'] = date('m/d/Y');
            $form_data['cs_title'] = 'President';

        } elseif ($document['contract_type'] === 'power_of_attorney') {
            $form_data['client_name'] = $enrollment['first_name'] . ' ' . $enrollment['last_name'];
            $form_data['client_address'] = trim(
                $enrollment['address_line1'] .
                ($enrollment['address_line2'] ? ', ' . $enrollment['address_line2'] : '') .
    ...
    foreach ($filled_pdfs as $filled_pdf_info) {
        ...
        } elseif ($pdf_type === 'power_of_attorney') {
            // Add signature on last page of power of attorney
            $signature_fields[] = [
                'page' => $pageCount,
                'x1' => 84.0,
                'y1' => 145.0,
        ...
        // If this is notice_of_cancellation and we're showing agreement type, add a DUPLICATE copy
        if ($pdf_type === 'notice_of_cancellation' && $contract_type === 'agreement') {
            // Add duplicate pages with watermark
            for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                $tplId = $final_pdf_obj->importPage($pageNo);
                $size = $final_pdf_obj->getTemplateSize($tplId);
                $final_pdf_obj->AddPage($size['orientation'], [$size['width'], $size['height']]);
                ...
                // Add DUPLICATE watermark at bottom right
                $final_pdf_obj->SetFont('Arial', 'B', 14);
                $final_pdf_obj->SetTextColor(180, 180, 180); // Light gray
                // Position at bottom right corner
                $pageWidthMM = $size['width'] * 0.352778;
                $pageHeightMM = $size['height'] * 0.352778;
                $final_pdf_obj->SetXY($pageWidthMM - 55, $pageHeightMM - 15);
                $final_pdf_obj->Cell(50, 10, 'DUPLICATE', 0, 0, 'R');
    ...
    error_log("Error generating contract preview: " . $e->getMessage());
