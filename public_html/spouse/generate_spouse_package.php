<?php
/**
 * Spouse Enrollment Package Background Processor
 * Generates the complete signed package with certificate for spouse
 */
...
// Get spouse enrollment ID and client info
$spouse_enrollment_id = intval($_POST['spouse_enrollment_id'] ?? 0);
$client_ip = $_POST['client_ip'] ?? '127.0.0.1';
$client_user_agent = $_POST['client_user_agent'] ?? 'Unknown Browser';

if ($spouse_enrollment_id <= 0) {
    error_log("Invalid spouse_enrollment_id");
    exit;
}

try {
    // Fetch spouse enrollment record
    $stmt = $pdo->prepare("SELECT * FROM spouse_enrollments WHERE id = ?");
    $stmt->execute([$spouse_enrollment_id]);
    $spouse_enrollment = $stmt->fetch();

    if (!$spouse_enrollment) {
        error_log("Spouse enrollment not found: $spouse_enrollment_id");
        exit;
    }

    if ($spouse_enrollment['package_status'] !== 'processing') {
        error_log("Spouse enrollment package already processed: $spouse_enrollment_id");
        exit;
    }
    ...
    if (!$enrollment) {
        throw new Exception("Primary enrollment not found");
    }

    $package_id = $enrollment['package_id'];
    $xactosign_package_id = $spouse_enrollment['spouse_xactosign_package_id'];
    ...
    error_log("Starting background processing for spouse enrollment: $spouse_enrollment_id, package: $xactosign_package_id");
    ...
    // Get SPOUSE signatures from contracts table (is_spouse = 1)
    $stmt = $pdo->prepare("SELECT contract_type, signature_data FROM contracts WHERE enrollment_id = ? AND is_spouse = 1 AND contract_type IN ('croa', 'client_agreement')");
    ...
    if (empty($signatures['croa']) || empty($signatures['client_agreement'])) {
        throw new Exception("Missing required spouse signatures");
    }
    ...
    // Prepare SPOUSE enrollment data
    $client_name = $enrollment['spouse_first_name'] . ' ' . $enrollment['spouse_last_name'];
    $client_address = trim(
    ...
    $client_phone = format_phone($enrollment['spouse_phone']);
    $client_email = $enrollment['spouse_email'];
    ...
    error_log("All spouse documents filled and signed");

    // Generate XactoSign certificate
    error_log("Generating XactoSign certificate for spouse");

    $certificate_data = [
        'company_name' => $company_name,
        'document_title' => $package['package_name'] . ' - Spouse Contract Package',
        ...
        'document_hash' => strtoupper(hash('sha256', 'spouse_package_' . $xactosign_package_id . '_' . time())),
    ...
    error_log("Spouse package generation complete. Total pages: $total_pages, Size: $file_size bytes");

    // Store in database
    $stmt = $pdo->prepare("UPDATE spouse_enrollments SET
        ...
        WHERE id = ?");
    $stmt->execute([$merged_pdf_content, $file_size, $total_pages, $spouse_enrollment_id]);

    error_log("Spouse package stored in database successfully");

    // Update CRC with final document ID for spouse
    if (!empty($spouse_enrollment['spouse_crc_record_id']) && !empty($spouse_enrollment['spouse_xactosign_package_id'])) {
        $memo = "--- SPOUSE ENROLLMENT COMPLETED ---\n";
        $memo .= "Document ID: " . $spouse_enrollment['spouse_xactosign_package_id'] . "\n";
        $memo .= "Package Pages: {$total_pages}\n";
        $memo .= "Completed: " . date('Y-m-d H:i:s');

        crc_update_memo($spouse_enrollment['spouse_crc_record_id'], $memo, 'Client');
        error_log("CRC updated with spouse document ID: " . $spouse_enrollment['spouse_xactosign_package_id']);
    }

    // Send Enrollment Complete notifications for SPOUSE (Client + Staff)
    ...
    $spouse_name = $enrollment['spouse_first_name'] . ' ' . $enrollment['spouse_last_name'];
    $primary_name = $enrollment['first_name'] . ' ' . $enrollment['last_name'];

    // Prepare notification data
    $notification_data = [
        'client_name' => $primary_name,
        'client_spouse_name' => $spouse_name,
        'client_first_name' => $enrollment['spouse_first_name'],
        'client_last_name' => $enrollment['spouse_last_name'],
        'client_email' => $enrollment['spouse_email'],
        'client_phone' => format_phone($enrollment['spouse_phone']),
        'plan_name' => $plan_name,
    ...
    // Send to spouse with PDF attachment
    $package_pdf_path = tempnam(sys_get_temp_dir(), 'spouse_package_') . '.pdf';
    ...
    send_notification('enrollment_complete_client', 'client', $spouse_recipient, $notification_data, $package_pdf_path);
    error_log("Enrollment Complete (Spouse Client) notification sent to " . $enrollment['spouse_email']);

    // Clean up temp PDF
    @unlink($package_pdf_path);

    // Send to staff with PDF attachment
    $staff_package_pdf_path = tempnam(sys_get_temp_dir(), 'spouse_package_staff_') . '.pdf';
    file_put_contents($staff_package_pdf_path, $merged_pdf_content);

    $staff_result = notify_staff('enrollment_complete_staff', 'notify_enrollment_complete', $notification_data, $staff_package_pdf_path);
    error_log("Enrollment Complete (Spouse Staff) notification sent to {$staff_result['staff_notified']} staff members");

    // Clean up temp PDF
    @unlink($staff_package_pdf_path);

} catch (Exception $e) {
    error_log("Error in spouse background package processing: " . $e->getMessage());
    ...
    try {
        $stmt = $pdo->prepare("UPDATE spouse_enrollments SET
            ...
            package_completed_at = NOW()
            WHERE id = ?");
        $stmt->execute([$e->getMessage(), $spouse_enrollment_id]);
